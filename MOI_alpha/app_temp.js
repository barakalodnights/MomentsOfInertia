...
function drawPolygon(state, vertices, closed) {
  ...
  ctx.fillStyle = closed ? 'rgba(37, 99, 235, 0.18)' : 'transparent';
...
function drawVertices(state, vertices) {
  const ctx = state.ctx;
  ctx.save();
...
function drawMomentIndicator(state) {
  ...
  const direction = appliedMoment >= 0 ? 1 : -1;
  ...
  ctx.lineTo(
    tipX - headSize * Math.cos(endAngle - direction * Math.PI / 7),
    tipY - headSize * Math.sin(endAngle - direction * Math.PI / 7)
  );
  ctx.moveTo(tipX, tipY);
  ctx.lineTo(
    tipX - headSize * Math.cos(endAngle + direction * Math.PI / 7),
  ...
  ctx.font = '12px Segoe UI';
  ctx.fillText(direction >= 0 ? 'CCW' : 'CW', center.x + radius + 8, center.y - 6);
  ctx.restore();
}
...
function drawState(state) {
  ...
  if (state.points.length > 0 || state.isClosed) {
    ...
    drawPolygon(state, vertices, state.isClosed);
    drawVertices(state, vertices);
  }
...
function updateInfoPanel() {
  momentDisplay.textContent = `${formatNumber(appliedMoment, 2)} N*m`;
